...
bool GLObjectObj::load_obj(const char* filename, vector<glm::vec3> &vertices, vector<glm::vec3> &normals, vector<glm::vec2> &textures, vector<GLuint> &elements)
{
	...
	if (temp_textures.size() == 0) {
		temp_textures.resize(temp_vertices.size());
	}
    /////////////////////////////////////////////
    ...
    for(int i=0; i<temp_index_triangle.size(); i++)
    {
		...
		glm::vec3 textureIdx = temp_index_textures[i];
		...
		textures.push_back(temp_textures[textureIdx.x - 1]);
		textures.push_back(temp_textures[textureIdx.y - 1]);
		textures.push_back(temp_textures[textureIdx.z - 1]);
...
void GLObjectObj::initVBO(void)
{
	...
	_num_texCoords = _textures.size();

    // create memory for the vertices, etc.
    float* vertices = new float[_num_vertices * 3 + (_num_texCoords * 2)];
    ...
    */
	...
	{
		glm::vec3 t = _vertices[i];
		glm::vec2 tex2 = _textures[i];
		for (int j = 0; j<3; j++) {
			vertices[(i * 5) + j] = t[j];
		}
		vertices[(i * 5) + 3] = tex2[0];
		vertices[(i * 5) + 4] = tex2[1];
	}
    ...
    //make 3 buffers, 1 additional for texture coordinates
    ...
    glBufferData(GL_ARRAY_BUFFER, ((_num_vertices * 3) + (_num_texCoords * 2)) * sizeof(GLfloat), vertices, GL_DYNAMIC_DRAW); // Set the size and data of our VBO and set it to STATIC_DRAW
    glEnableVertexAttribArray(locPos); //
    glVertexAttribPointer((GLuint)locPos, 3, GL_FLOAT, GL_FALSE, 5*sizeof(GLfloat), NULL); // Set up our vertex attributes pointer
    
    
	int locTex = glGetAttribLocation(_program, "in_TexCoord");
	glEnableVertexAttribArray(locTex);
	glVertexAttribPointer((GLuint)locTex, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), (const GLvoid*)(3 * sizeof(GLfloat))); // Set up our vertex attributes pointer
